...
Imagine we're dividing two 8-bit unsigned integers: $`227`$ and $`5`$, which are $`111001_2`$ and $`101_2`$ in binary. Integer division result is $`45`$ ( keep in mind that $`227`$ is not really divisible by $`5`$ ), and now we'll find it with binary search:
...
So, we've got answer $`199`$. Well, this is wrong obviously. But why?
Well, if you mutliply $`199`$ and $`5`$ as 8-bit integers - you'll see $`227`$ as the result.

Overflow is what we need to get rid of.

Let's take a closer look: we get overflow each time when we're trying to multiply $`5`$ with anything bigger than $`51`$. Overflow means that multiplication is guaranteed to be greater $`227`$, and we need to relax binsearch upper limit.

But how to do this?

Well, when you multiply by $`5`$, you actually multiply by $`101_2`$. From that point, we can try to limit result ( for $`x > 0`$ ):
...
So, let's set upper limit to $`63`$ for binsearch and try original example one more time:
...
Our answer is $`45`$ now, because $`225 \le 227 \le 230`$
...
We've just got overflow, again. But it's actually easy to deal with it: if $`c \cdot b`$ ( in this case $`23 \cdot 15 = 89`$ ) is smaller than range start times $`b`$ ( in this case for range $`\left[16; 31\right)`$ is's $`16 \cdot 15 = 240`$ ), then we've got overflow. But why don't we taken this approach earlier? Because it was not guaranteed that overflowed value would be less than range start times $`b`$.
...
Every overflowed value is less than $`240`$. That's the point.
